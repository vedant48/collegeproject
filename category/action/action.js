...
function showMovies(data) {
    ...
    data.forEach(movie => {
        const {title, name, poster_path, vote_average, overview, id, imdb_id} = movie;
  ...
  fetch(BASE_URL + '/movie/'+id+'/videos?'+API_KEY).then(res => res.json()).then(videoData => {
    ...
    if(videoData){
      ...
      if(videoData.results.length > 0){
        ...
        fetch(BASE_URL + '/movie/'+id+'?'+API_KEY).then(res => res.json()).then(movieEach => {
          console.log(movieEach);
          let {title, name, poster_path, vote_average, overview, id, imdb_id} = movieEach;
        var content = `
